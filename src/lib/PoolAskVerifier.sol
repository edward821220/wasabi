...
library PoolAskVerifier {
    ...
    bytes32 constant POOLASK_TYPEHASH = keccak256(
     ...
     */
    function hashForPoolAsk(WasabiStructs.PoolAsk memory _poolAsk) public pure returns (bytes32) {
        return keccak256(
            abi.encode(
                POOLASK_TYPEHASH,
                _poolAsk.id,
     ...
     */
    function getSignerForPoolAsk(WasabiStructs.PoolAsk memory _poolAsk, bytes memory _signature)
        public
        view
        returns (address)
    {
        ...
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, hashForPoolAsk(_poolAsk)));
     ...
     */
    function verifyPoolAsk(WasabiStructs.PoolAsk memory _poolAsk, bytes memory _signature, address _signer)
        internal
        view
        returns (bool)
    {
