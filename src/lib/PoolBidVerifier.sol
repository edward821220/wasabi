...
library PoolBidVerifier {
    ...
    bytes32 constant POOLBID_TYPEHASH =
        keccak256("PoolBid(uint256 id,uint256 price,address tokenAddress,uint256 orderExpiry,uint256 optionId)");
     ...
     */
    function hashForPoolBid(WasabiStructs.PoolBid memory _poolBid) public pure returns (bytes32) {
        return keccak256(
            abi.encode(
                POOLBID_TYPEHASH,
                _poolBid.id,
     ...
     */
    function getSignerForPoolBid(WasabiStructs.PoolBid memory _poolBid, bytes memory _signature)
        public
        view
        returns (address)
    {
        ...
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, hashForPoolBid(_poolBid)));
     ...
     */
    function verifyPoolBid(WasabiStructs.PoolBid memory _poolBid, bytes memory _signature, address _signer)
        internal
        view
        returns (bool)
    {
