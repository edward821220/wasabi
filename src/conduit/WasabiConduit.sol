 ...
 */
contract WasabiConduit is Ownable, IERC721Receiver, ReentrancyGuard, ConduitSignatureVerifier, IWasabiConduit {
    ...
    function buyOptions(
        ...
        uint256[] memory optionIds = new uint256[](size);
    ...
    /// @inheritdoc IWasabiConduit
    function buyOption(WasabiStructs.PoolAsk calldata _request, bytes calldata _signature)
        public
        payable
        returns (uint256)
    {
    ...
    /// @inheritdoc IWasabiConduit
    function acceptAsk(WasabiStructs.Ask calldata _ask, bytes calldata _signature)
        public
        payable
        nonReentrant
        returns (uint256)
     ...
     */
    function validateAsk(WasabiStructs.Ask calldata _ask, bytes calldata _signature) internal view {
        // Validate Signature
        address currentOwner = option.ownerOf(_ask.optionId);

        require(
            verifyAsk(_ask, _signature, owner()) || verifyAsk(_ask, _signature, currentOwner), "Incorrect signature"
        );
     ...
     */
    function validateOptionForBid(uint256 _optionId, IWasabiPool _pool, WasabiStructs.Bid calldata _bid)
        internal
        view
    {
        ...
        uint256 diff =
            optionData.expiry > _bid.expiry ? optionData.expiry - _bid.expiry : _bid.expiry - optionData.expiry;
     ...
     */
    function validateBid(IWasabiPool _pool, WasabiStructs.Bid calldata _bid, bytes calldata _signature) internal view {
        // Validate Signature
        require(verifyBid(_bid, _signature, owner()) || verifyBid(_bid, _signature, _bid.buyer), "Incorrect signature");
     ...
     */
    function getAskId(WasabiStructs.Ask calldata _ask) internal pure returns (bytes memory) {
     ...
     */
    function getBidId(WasabiStructs.Bid calldata _bid) internal pure returns (bytes memory) {
