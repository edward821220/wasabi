...
abstract contract ConduitSignatureVerifier {
     ...
     */
    function hashDomain(WasabiStructs.EIP712Domain memory _eip712Domain) internal pure returns (bytes32) {
        return keccak256(
            abi.encode(
     ...
     */
    function hashForBid(WasabiStructs.Bid memory _bid) public pure returns (bytes32) {
        return keccak256(
            abi.encode(
                BID_TYPEHASH,
                _bid.id,
     ...
     */
    function hashForAsk(WasabiStructs.Ask memory _ask) public pure returns (bytes32) {
        return keccak256(
            abi.encode(
                ASK_TYPEHASH, _ask.id, _ask.price, _ask.tokenAddress, _ask.orderExpiry, _ask.seller, _ask.optionId
            )
        );
    }
     ...
     */
    function getSignerForBid(WasabiStructs.Bid memory _bid, bytes memory _signature) public view returns (address) {
        ...
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, hashForBid(_bid)));
     ...
     * @return address who signed the signature
     */
    function getSignerForAsk(WasabiStructs.Ask memory _ask, bytes memory _signature) public view returns (address) {
        bytes32 domainSeparator = hashDomain(
            WasabiStructs.EIP712Domain({
                name: "ConduitSignature",
                version: "1",
        ...
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, hashForAsk(_ask)));
        return Signing.recoverSigner(digest, _signature);
    }
     ...
     */
    function verifyBid(WasabiStructs.Bid memory _bid, bytes memory _signature, address _signer)
        internal
        view
        returns (bool)
    {
     ...
     * @return true if the signature belongs to the signer, false otherwise
     */
    function verifyAsk(WasabiStructs.Ask memory _ask, bytes memory _signature, address _signer)
        internal
        view
        returns (bool)
    {
